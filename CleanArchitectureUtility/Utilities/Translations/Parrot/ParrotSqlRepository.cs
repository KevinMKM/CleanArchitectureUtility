...
public class ParrotSqlRepository
{
    ...
    private void LoadLocalizationRecords(object? state)
    {
        lock (Locker)
        {
            _logger.LogInformation($"Parrot Translator load localization recorded at {DateTime.Now}");
            _localizationRecords = _dbConnection.Query<LocalizationRecord>(_selectCommand, commandType: CommandType.Text).ToList();
            _logger.LogInformation($"Parrot Translator loaded localization recorded at {DateTime.Now}. Total record count is {_localizationRecords.Count}");
        }
    }
    ...
    private void SeedData()
    {
        ...
        if (count <= 0) 
            return;
        var command = $"INSERT INTO [{_configuration.SchemaName}].[{_configuration.TableName}]([Key],[Value],[Culture]) VALUES {string.Join(",", newItemsInDefaultTranslations)}";
        using IDbConnection db = new SqlConnection(_configuration.ConnectionString);
        db.Execute(command, commandType: CommandType.Text);
        _logger.LogInformation($"Parrot Translator Add {count} items to its dictionary from default translations at {DateTime.Now}");
    }
