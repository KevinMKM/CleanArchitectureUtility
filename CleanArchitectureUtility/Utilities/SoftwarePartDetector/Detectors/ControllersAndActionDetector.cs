...
public class ControllersAndActionDetector
{
    ...
    public Task<List<SoftwarePartController>> Detect()
    {
        return Task.Run(() =>
        {
            ...
            {
                var attribute =
                    item.ControllerTypeInfo.GetCustomAttributes(typeof(SoftwarePartControllerOptionAttribute), false).FirstOrDefault() as SoftwarePartControllerOptionAttribute;
            ...
            result = result.DistinctBy(c => c.Name)
                .GroupJoin(controllerActionDescriptors, c => c.Name, a => a.ControllerName, (c, a) => new SoftwarePartController
                {
                    Module = c.Module,
                    Service = c.Service,
                    Name = c.Name,
