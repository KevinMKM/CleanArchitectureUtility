...
using CleanArchitectureUtility.Utilities.Auth.ApiAuthentication.Models;
...
public static class ProviderExtensions
{
    public static string AddProviderHttpClient(this ProviderOption provider, IServiceCollection services)
    {
        ...
        var httpClientBuilder = services.AddHttpClient(httpClientName, option =>
        {
            option.BaseAddress = new Uri(provider.Authority);
        });
    ...
    public static async Task<List<Claim>> UserInfoEndpointCaller(this ProviderOption provider, HttpContext httpContext, HttpClient client, string token)
    {
        ...
        if (provider.RegisterUserInfoClaims.CachingData)
        {
            ...
            claims = cacheAdapter.Get<List<ClaimCacheModel>>(cacheKey)?.Select(cacheModel => cacheModel.ToClaim())
            ...
            switch (provider.RegisterUserInfoClaims.CacheExpirationType)
            {
                ...
                default:
                    throw new InvalidOperationException(
                        $"Invalid cache expiration type for {provider.Scheme} ({provider.Authority})");
            }
        }
        else
            claims = await provider.CallUserInfoEndpoint(client, token);
    ...
    private static string GenerateCacheKey(this ProviderOption provider, string token)
    {
        ...
        var cacheKey = provider.RegisterUserInfoClaims.CacheKeyFormat == CacheKeyFormat.Base64
            ? cacheKeyText.ToBase64Encode()
            : cacheKeyText;
        return cacheKey;
    }
    ...
    public static async Task<List<Claim>> CallUserInfoEndpoint(this ProviderOption provider, HttpClient client, string token)
    {
        ...
        {
            Address = provider.EndpointsPath?.UserInfoEndpoint ??
                      (await client.GetDiscoveryDocumentAsync())?.UserInfoEndpoint?.Replace(provider.Authority, ""),
            Token = token
        });
